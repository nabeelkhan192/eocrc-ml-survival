# Protocol-to-Code Concordance (Version 4.1.1)
...
repository claims they have already run. Rows marked *NOT yet executed*
are prespecified but await the post-registration Stage-02 gate; they are
not claimed as completed analyses.
...
| DCA, exploratory 5-60% range, horizon-linked | §2, §7 | config.DCA_THRESHOLDS, HORIZON_MONTHS (06 reads the locked horizon); src/06_decision_curves.py |
...
| Events-per-parameter check / feature-reduction order | §9 | *real-data stage, NOT yet executed*; evaluated at the Stage-02 gate from 02 outputs; deterministic reduction order fixed in §9 (stage, nodes, histology, grade, size, age) |
...
| Overall survival: descriptive only (KM); no OS models prespecified | §2 | 01 os_event; 02 os_plot -> km_os_eo_vs_ao.png (implemented) |
| Integrated Brier score | — | NOT prespecified (removed v4.0; documented) |
| Sensitivity analyses (2)-(6): complete-case, rectal-excluded, 2019-excluded, COVID extension, stage I-III subgroup | §8 | *real-data stage, NOT yet executed*: prespecified cohort-filter re-runs of 01->07; no dedicated script — run manually with the documented filters |
